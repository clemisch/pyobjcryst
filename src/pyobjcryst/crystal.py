...
class Crystal(Crystal_orig):
    ...
    def _display_cif(
        ...
        """Create a CIF with the full list of atoms, including those
        deduced by symmetry or translation up to neighbouring unit
        cells.
    ...
    def _display_list(
        ...
        """Create a list of atoms to be displayed, so it can be supplied
        to py3Dmol.
            ...
            if scatt.GetClassName() == "Molecule":
                ...
                if only_independent_atoms:
                    ...
                    for a in atoms.values():
                        ...
                        x, y, z = self.FractionalToOrthonormalCoords(
                            a["x"], a["y"], a["z"]
                        )
                        vv.append(
                            {
    ...
    def display_3d(
        ...
        zmax=1,
        enantiomer=False,
        full_molecule_opacity=0.5,
        extra_dist=2,
        extra_opacity=0.5,
    ):
        """This will return a 3D view of the Crystal structure which can
        be displayed in a notebook. This cannot be automatically
        updated, but will remain in the notebook as a static javascript
        object, so it can still be useful.
        ...
        if py3Dmol is None:
            warnings.warn(
                "Yout need to install py3Dmol>=0.9 to use Crystal.display_3d()"
            )
            return
        v = py3Dmol.view()
        ...
        if full_molecule_opacity > 0:
            ...
            m.setStyle(
                {
                    "stick": {"radius": 0.2, "opacity": full_molecule_opacity},
                    "sphere": {
                        "scale": 0.3,
                        "colorscheme": "jmol",
        ...
        if extra_opacity > 0 and extra_dist > 0:
            ...
            m.setStyle(
                {
                    "stick": {"radius": 0.2, "opacity": extra_opacity},
                    "sphere": {
                        "scale": 0.3,
                        "colorscheme": "jmol",
    ...
    def widget_3d(
        ...
        """This will return a 3D view of the Crystal structure which can
        be displayed in a notebook, along with controls for the display.
        This can be live-updated.
        ...
        if widgets is None or py3Dmol is None:
            warnings.warn(
                "You need to install py3Dmol>=0.9 and ipywidgets to use Crystal.widget_3d()"
            )
            return
        ...
        xstep = 0.5 / self.a
        # Adapt step so we can keep original values as integral number steps
        xstep = (xmax - xmin) / np.ceil((xmax - xmin) / xstep)
        self.xrange = widgets.FloatRangeSlider(
            value=[xmin, xmax],
            min=xmin - 0.5,
            max=xmax + 0.5,
            step=xstep,
        ...
        self.yrange = widgets.FloatRangeSlider(
            value=[ymin, ymax],
            min=ymin - 0.5,
            max=ymax + 0.5,
            step=ystep,
        ...
        self.zrange = widgets.FloatRangeSlider(
            value=[zmin, zmax],
            min=zmin - 0.5,
            max=zmax + 0.5,
            step=zstep,
        ...
        self.extra_dist = widgets.FloatSlider(
            value=extra_dist,
            min=0,
            max=10,
            step=0.5,
        ...
        self.extra_opacity = widgets.FloatSlider(
            value=extra_opacity,
            min=0,
            max=1,
            step=0.1,
        ...
        self.full_molecule_opacity = widgets.FloatSlider(
            value=full_molecule_opacity,
            min=0,
            max=1,
            step=0.1,
        ...
        self.vbox_options = widgets.VBox(
            [self.extra_dist, self.extra_opacity, self.full_molecule_opacity]
        )
    ...
    def _widget_update(self, show=False, zoom=False):
        ...
        if show:
            ...
            # This avoids adding extra lines in the javascript output every time
...
def create_crystal_from_cif(
    ...
    :param multiple: if True, all structures from the CIF will be imported, but
        the returned Crystal object and those created in the global registry
